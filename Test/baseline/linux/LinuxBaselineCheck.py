...
def linux_scan_res_report(scanResDict={}):
    ...
    auditdRulesNotCheckedUserandgroupfile = auditd_rules_info[
    ...
    auditdRulesNotCheckedDACChangeSyscall = auditd_rules_info[
        'auditdRulesNotCheckedDACChangeSyscall']
    if auditdRulesIfNotNull == "True" and len(auditdRulesNotCheckedDACChangeSyscall) == 0:
        pass_Num += 1
    auditdRulesCheckedFileAccessAttemptSyscall = auditd_rules_info[
        'auditdRulesCheckedFileAccessAttemptSyscall']
    auditdRulesNotCheckedFileAccessAttemptSyscall = auditd_rules_info[
        'auditdRulesNotCheckedFileAccessAttemptSyscall']
    ...
    auditdRulesNotCheckedPrivilegedCommand = auditd_rules_info[
        'auditdRulesNotCheckedPrivilegedCommand']
    ...
    if "False" not in userIfOwnTheirHomeDirArray:
        pass_Num += 1
    userHomeDirIfHasGWorOWDotFileArray = usergroup_config_info[
        'userHomeDirIfHasGWorOWDotFileArray']
                ...
                try:
                    if response['results'][i]['vulnerabilities'][j]['exploits']:
                        ...
                        for z in range(0, len(response['results'][i]['vulnerabilities'][j]['exploits'])):
                            ...
                            print(bcolors.OKGREEN + "[+] " + bcolors.ENDC + "Title: " +
                                  response['results'][i]['vulnerabilities'][j]['exploits'][z]['title'])
                            print(
                                bcolors.FAIL + "[!] Exploit ID: EDB" + edb[2] + bcolors.ENDC + "\n")
