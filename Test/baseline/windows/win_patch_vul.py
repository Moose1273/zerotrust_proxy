...
WEB_URL = ''
BANNER = '%s %s ( %s )'
FILENAME = 'windows/win_patch_vul.py'
...
def windows_os_vul_res_report(systeminfo):
    ...
    if hasattr(args, 'perform_update') and args.perform_update:
        ...
        urlretrieve('https://raw.githubusercontent.com/bitsadmin/wesng/master/definitions.zip', 'windows/definitions.zip')
        cves, date = load_definitions('windows/definitions.zip')
    ...
    cves = None
    systeminfo_data = open(systeminfo, 'rb').read()
    try:
...
def parse_arguments():
    ...
    # General options
    parser.add_argument('--definitions', action='store', nargs='?', type=check_definitions_exists, default='windows/definitions.zip', help='Definitions zip file (default: windows/definitions.zip)')
    ...
    # if len(sys.argv) == 1:
        #parser.print_help()
        #print("using --update or input a systeminfo file\n")
        #exit(1)
...
if __name__ == '__main__':
    windows_os_vul_res_report('systeminfo.txt') 
