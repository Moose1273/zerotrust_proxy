#显示utf8编码的中文字符
chcp 65001
$InventoryOutFile = 'windows/win_res.json'
$KBOutFile = 'windows/KB.json'
$SecOutFile = 'windows/sec.inf'
...
function Get-SecInfo{
	$secinfo=""
	SecEdit.exe /export /cfg $SecOutFile /Quiet
	$secInfoArray=Get-Content .\$SecOutFile
...
$KBResult|Out-File $KBOutFile -encoding utf8
...
Write-Output  $window_check_res
#Invoke-RestMethod -Uri "http://192.168.163.129:8888/baseline/windows_scan_res_report/" -Method Post -ContentType "application/json" -Body $window_check_res
Write-Host "Press any key to continue..."

#echo 默认为utf-16-le编码方式
#echo $window_check_res > win_res.json 

#指定为utf-8-sig编码方式
$window_check_res | Out-File -Encoding UTF8 -FilePath $InventoryOutFile
